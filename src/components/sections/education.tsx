...
import { EDUCATION, UI_TEXT } from "@/app/constants/data";
    ...
    <section id="education">
      <div className="flex items-center py-2 px-4 border-b border-dashed border-border">
        <h2 className="text-2xl font-semibold flex items-center">
          {UI_TEXT.sections.education[lang]}
        </h2>
      </div>
        ...
        {EDUCATION.map((edu, index) => (
          <div key={index} className="space-y-1 p-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
              <h3 className="font-medium text-base">{edu.degree[lang]}</h3>
            ...
            <p className="text-sm text-muted-foreground">
              {UI_TEXT.sections.scoreLabel[lang]}{edu.score[lang]}
            </p>
          </div>
        ))}
      </div>
